							...
							<div class="span14" id="reminderSearchArea">
								搜索: <input type="text" style="width:200px;" id="keyStr" value="${requestScope.keyStr }" placeholder="客户电话/姓名/等级/签单贷款类型" aria-controls="customerTable">
									<select id="fllowType" class="span2" style="width:100px;">
										...
										<option value="100"<c:if test="${fllowType==100 }"> selected="selected"</c:if>>合同签订</option>
										<option value="101"<c:if test="${fllowType==101 }"> selected="selected"</c:if>>合同审批</option>
										<option value="102"<c:if test="${fllowType==102 }"> selected="selected"</c:if>>抵押办理</option>
										<option value="103"<c:if test="${fllowType==103 }"> selected="selected"</c:if>>放款</option>
										<option value="104"<c:if test="${fllowType==104 }"> selected="selected"</c:if>>其他</option>
									</select>
								<c:if test="${requestScope.roleId==2000 || requestScope.roleId==2001}">
								...
								<c:if test="${requestScope.roleId==2000 || requestScope.roleId==2001 ||requestScope.roleId==2002}">
	...
	$(function() {
		...
		$("#keyStr").keyup(function(event){
			 if(event.keyCode == 13){
				...
				$("#tableList").load("warrant/getReminderList.do?initPage=1&pageIndex=1&keyword="
