...
<table
	...
	<thead>
		<tr>
			...
			<td style="min-width: 48px;">合同名称</td>
			<td style="min-width: 38px;">合同编号</td>
			<td style="min-width: 48px;">客户项目授权人</td>
			<td style="min-width: 48px;">项目负责人</td>
			<td style="min-width: 38px;">签订时间</td>
			<td style="min-width: 48px;">合同附件</td>
			<!--  
			<c:if
	...
	<tbody>
		<c:forEach items="${requestScope.customers}" var="cust"
			varStatus="vsta">
			<tr cId="${cust.id}" hId="${cust.id}">
				<td>${cust.id }</td>
				<td class="hidden-480">${cust.itemName}</td>
				<td><a href="javascript:void(0);" title="${cust.itemNo}"
					style="text-decoration: none; color: #000000">
						<div>${cust.itemNo }</div>
				</a></td>
				<td><a href="javascript:void(0);"
					title="${cust.custAuthorizedPer}"
					style="text-decoration: none; color: #000000">
						<div>${cust.custAuthorizedPer}</div>
				</a></td>

				<td class="hidden-480">${cust.projectLeader}</td>
				<td class="hidden-480">${cust.createTime }</td>
				<td class="hidden-480"><a
					href="javascript:downloadPic('${cust.id}')" id="pluginurl">下载</a></td>
				<!-- 
				<td class="hidden-480">
...
<script>
	...
	console.log("有效客户列表双击tr");
		...
		$("#validCustomerDetail").load(
			"${pageContext.request.contextPath}/contractManager/updateContractManaList.do?id="
