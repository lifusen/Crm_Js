	...
	<div class="row-fluid">
		<div class="span12">
			...
			<ul class="breadcrumb">
				...
				<li><a href="javascript:void(0);">退单退件列表</a></li>
			</ul>
		</div>
	</div>
		...
		<div class="span12">
			...
			<div class="portlet box yellow">
				<div class="portlet-title">
					<div class="caption"><i class="icon-globe"></i>退单退件列表</div>
					<div class="tools">
						...
						<div class="row-fluid">
							...
							搜索: <input type="text" style="width:300px;" id="keyStr" value="${requestScope.keyStr }" placeholder="客户电话/姓名/等级/签单贷款类型" aria-controls="customerTable">
								<select id="signState" style="width:100px;">
									...
									<option value="1" <c:if test="${requestScope.state == 1 }"> selected="selected"</c:if>>退单</option>
									<option value="2" <c:if test="${requestScope.state == 2 }"> selected="selected"</c:if>>退件</option>
								</select>
								<select id="lastFailureMessageCause" style="width:100px;">
									<option value="">所有原因</option>
									<option value="不接受利息" <c:if test="${requestScope.failureMessage == '不接受利息' }"> selected="selected"</c:if>>不接受利息</option>
									<option value="客户不做了" <c:if test="${requestScope.failureMessage == '客户不做了' }"> selected="selected"</c:if>>客户不做了</option>
									<option value="无渠道/不符合条件" <c:if test="${requestScope.failureMessage == '无渠道/不符合条件' }"> selected="selected"</c:if>>无渠道/不符合条件</option>
									<option value="额度达不到客户要求" <c:if test="${requestScope.failureMessage == '额度达不到客户要求' }"> selected="selected"</c:if>>额度达不到客户要求</option>
									<option value="客户不配合补资料" <c:if test="${requestScope.failureMessage == '客户不配合补资料' }"> selected="selected"</c:if>>客户不配合补资料</option>
									<option value="客户联系不上" <c:if test="${requestScope.failureMessage == '客户联系不上' }"> selected="selected"</c:if>>客户联系不上</option>
									<option value="其他" <c:if test="${requestScope.failureMessage == '其他' }"> selected="selected"</c:if>>其他</option>
								</select>
								...
								时间：<input data-provide="datepicker" type="text" class="span2 small" id="beginDate" value="${requestScope.beginDate }" readonly="readonly" size="12"  style="width:100px;"/>
	...
	$(function(){
		...
		$("#keyStr").keyup(function(event){
			 if(event.keyCode == 13){
				...
				$("#tableList").load("rejectCustomer/getRejectCustomerList.do?initPage=1&pageIndex=1&keyword="+val+"&state="
						+state+"&dId="+dId+"&eId="+eId+"&beginDate="+beginDate+"&endDate="+endDate+"&failureMessage="+failureMessage+"&pageSize="+$("#pageSize").val());
			 }
		 });
