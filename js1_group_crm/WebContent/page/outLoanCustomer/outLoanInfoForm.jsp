...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>  
	<c:set var="olc" value="${outLoanCustomer}"/>
	<form id="updateOutLoanCustomerForm" action="">
		<input type="hidden" name="customerId" value="${olc.signCustomer.customer.id }"/>
		<%-- <input type="hidden" name="sId" value="${sc.id }"/>
		<input type="hidden" name="oId" value="${olc.id }"/> --%>
	...
	<tr>
		...
		<td>
			<div id="showRateOfAddOutLoanCustomer"><label class="control-label"></label></div>
				<div id="choiceRateOfAddOutLoanCustomer" style="display:none;">
				<select id="rateSelectOfAddOutLoanCustomer" name="loanProducts[0].rateType" class="small" style="width:50px;height:26px;">
			...
			<select name="loanProducts[0].loanYear" class="" style="width:70px;">
				...
				<optgroup label="月">
					<c:forEach begin="1" end="12" var="i">
						...
						<option value="${loanMonthTemp}" ${olc.loanProducts[0].loanYear==loanMonthTemp?'selected':''}>${i}月</option>
					</c:forEach>
				</optgroup>
				<optgroup label="年">
	...
	<tr>
		<td><label class="control-label">上 门 费</label></td>
		<td>
			<input type="text" name="visitCharge" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.visitCharge }"/>"/> 
		</td>
		<td><label class="control-label">渠道成本</label></td>
		<td>
			<input type="text" name="channelCost" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.channelCost }"/>"/> 
		</td>
		<td><label class="control-label">其 他 费</label></td>
		<td>
			<input type="text" name="otherCost" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.otherCost }"/>"/> 
		</td>
		<td><label class="control-label">返&nbsp;点</label></td>
		<td>
			<input type="text" name="rebate" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.rebate }"/>"/> 
		</td>
	</tr>
	
	<tr style="border-top:1px dashed green;">
		<td><label class="control-label">用&nbsp;途</label></td>
		<td>
			<input type="text" name="purpose" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.purpose }"/>"/> 
		</td>
		<td><label class="control-label">征 信 费</label></td>
		<td>
			<input type="text" name="creditCost" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.creditCost }"/>"/> 
		</td>
		<td><label class="control-label">电&nbsp;票</label></td>
		<td>
			<input type="text" name="electricCost" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.electricCost }"/>"/> 
		</td>
	</tr>
	<tr>
		<td><label class="control-label">工作证明</label></td>
		<td>
			<input type="text" name="workProve" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.workProve }"/>"/> 
		</td>
		<td><label class="control-label">快 递 费</label></td>
		<td>
			<input type="text" name="postage" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.postage }"/>"/> 
		</td>
		<td><label class="control-label">面 签 费</label></td>
		<td>
			<input type="text" name="checkCost" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.checkCost }"/>"/> 
		</td>
	</tr>
	<tr>
		<td><label class="control-label">座 机 费</label></td>
		<td>
			<input type="text" name="telephoneCost" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.telephoneCost }"/>"/> 
		</td>
		<td><label class="control-label">电 话 卡</label></td>
		<td>
			<input type="text" name="phoneCard" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.phoneCard }"/>"/> 
		</td>
		<td><label class="control-label">纯 处 理</label></td>
		<td>
			<input type="text" name="onlyHandle" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.onlyHandle }"/>"/> 
		</td>
	</tr>
	
	<tr style="border-top:1px dashed green;">
		<td><label class="control-label">房&nbsp;龄</label></td>
		<td>
			<input type="text" name="houseAge" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.houseAge }"/>"/> 
		</td>
	...
	<tr>
		...
		<td><label class="control-label">保 险 费</label></td>
		<td>
			<input type="text" name="insuranceCost" class="input width120" value="<fmt:formatNumber pattern="#.#" value="${olc.insuranceCost }"/>"/> 
		</td>
	</tr>
	...
	<tr>
		...
		<td colspan="8">
			<button id="updateOutLoanCustomerImportCertificate" type="button" class="btn blue" onclick="importCertificate(this,${sc.customer.id})"><i class="icon-plus"></i> 导入影像文件</button>
		</td>
	</tr>
	<tr>
		<td><label class="control-label">抵押物</label></td>
		<td style="width:60px;">
			<c:forEach var="cvb" items="${olc.signCustomer.collateralViewBeans}">
	...
	<c:if test="${refererUrl != null && not empty refererUrl}">
		<a class="btn aPost" href="${refererUrl}"><i class="icon-remove"></i> 取消</a>
	</c:if>
	<c:if test="${refererUrl == null || empty refererUrl}">
		<a class="btn" href="javascript:void(0);" id="hideDetailInOut"><i class="icon-remove"></i> 取消</a>
	</c:if>
	</div>
...
//激活导入影像文件
$("#updateOutLoanCustomerImportCertificate").on("click",updateOutLoanCustomerImportCertificate);
function updateOutLoanCustomerImportCertificate(){
	importCertificate(this,"${olc.signCustomer.customer.id}");
}
...
$(function(){
	...
	//默认显示
	$('#showRateOfAddOutLoanCustomer label').html($('#rateSelectOfAddOutLoanCustomer option:selected').text());
	
	//单击利率
	$('#showRateOfAddOutLoanCustomer').click(function(){
		$(this).hide();
		$('#choiceRateOfAddOutLoanCustomer').show();
	});
	
	$('#rateSelectOfAddOutLoanCustomer').change(function(){
		$('#choiceRateOfAddOutLoanCustomer').hide();
		$('#showRateOfAddOutLoanCustomer label').html($('#rateSelectOfAddOutLoanCustomer option:selected').text());
		$('#showRateOfAddOutLoanCustomer').show();
	});
